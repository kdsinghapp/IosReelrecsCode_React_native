...
const CommentModal: React.FC<Props> = ({ visible, onClose, reviews,
  ...
  console.log('has_rated_movie__PO_', has_rated_movie, has_rated_movie_Ref.current)
  ...
  // console.log(userData  , 'frggr__hghgh')
  // console.log(userData , 'suerNmae_comment__')
  console.log(imdb_id, "userData__comment")
  ...
  // console.log(userprofile , 'ergerg_gfge')
  ...
  useEffect(() => {
    if (visible && reviews && comments.length === 0) {
      ...
      // console.log(reviews, "fbhfhdeh_nhj")
    }
  }, [visible, reviews]);

  useEffect(() => {
    if (visible) {
      console.log('useefect__data___call___yeHUa')
      setPage(1);
      setHasMore(true);
      fetchComments(1); // fetch first page
    }
  ...
  const fetchComments = async (nextPage = 1) => {
    ...
    try {
      ...
      console.log('call___getCommentsByMovie', imdb_id, 'data___from__getCommentsByMovie',
  ...
  const handlePostComment = async (text_Touch?: boolean) => {
    console.log('has_rated_movie_Ref__current____', has_rated_movie_Ref.current)
    if (!has_rated_movie_Ref.current) {
      showCommenRankingCheck()
      // text_Touch_ref.current  = true
      return
    }
    console.log('call_data__here', has_rated_movie_Ref.current)
    ...
    setComments(prev => {
      ...
      if (userData?.username) {
        ...
        // console.log('asef_jjhh',userIndex)
      }
  ...
  // console.log(imdb_id  , 'comment_for_movie__here')

  useEffect(() => {
    // console.log('see_change__imdb_id',imdb_id)
    setCommentText('')
    setComments([])
