...
const handleSave = async () => {
  ...
  if (fieldKey === 'username') {
    try {
      ...
      if (!result.available) {
        console.log('❗️Username Taken', 'Please try another username');
        return;
      }
    } catch (error) {
  ...
  if (fieldKey === 'group_name' && groupId && group_name && trimmedValue !== group_name) {
    ...
    const response =   await renameGroup(token, groupId, trimmedValue);
      ...
      if (response) {
        ...
        // console.log(response , "response.status_code________response.status_code")
      }
    } catch (error) {
      console.log('❌ Group rename failed:', error);
    }
  }
