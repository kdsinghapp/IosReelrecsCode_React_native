...
const SearchMovieCom = ({
  ...
  //     const updatedList = movieData.map((movie) =>
  //       movie.imdb_id === imdb_id ? { ...movie, saved: updatedSaved } : movie
  //     );
  //     setSearchData(updatedList); // UI Update
  //   } catch (error) {
  ...
  const renderMovie = useCallback(({ item }) => {
    ...
    console.log(item, "Seraach__oitem__Here")
    return (
      <TouchableOpacity
      ...
      >
        ...
        <TouchableOpacity style={styles.info} onPress={() => {
          ...
          // console.log(item, 'item___searchMovie__ITEM')
        }} >
  ...
  }, [navigation, togglePlatform, setIsVisible, movieData]);
          ...
          {loading ? (
            ...
            <View style={styles.noResultsContainer}>
              <Text style={styles.noResultsText}>
                No results found{formattedQuery ? ` for "${formattedQuery}"` : ''}
              </Text>
            </View>
          ) : (
            <FlatList
