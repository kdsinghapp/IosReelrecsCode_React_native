  ...
  for (let i = 0; i < segmentLines.length; i++) {
    ...
    if (!res.ok) {
      ...
      if (!ok) {
        ...
        console.warn('Segment failed after retries:', segUrl);
      }
    }
...
export async function getCacheSize() {
  try {
    ...
    console.warn('getCacheSize error', e);
    return 0;
  }
}
...
console.log('total___enforceCacheLimit___----',total)
  // sort by oldest
  ...
  for (const f of info) {
    ...
    try {
      ...
      console.warn('Failed to delete cache folder', f.path, e);
    }
  }
