...
useEffect(() => {
  ...
  const fetchNextMovieFromQueue = async (prevImdb: any) => {
...
console.log('before__data__preloadNeighborsMeta','queue' ,queue ,  'meta__',meta)
    ...
    if (newIndex !== currentIndex && !isResettingRef.current) {
      if (newIndex === 0 || newIndex === 2) {
        isResettingRef.current = true; // prevent multiple triggers
         setIsLoadingNewMovie(true); // 🟡 show shimmer immediately
        ...
        // setMovieData([null, null, null]); // comment
        setProgress(0);
        ...
        if (newMovie) {
          ...
          setTimeout(() => {
            ...
            setIsLoadingNewMovie(false); // 🟢 stop shimmer
          }, 500);
        } else {
  ...
  // example: after you set new mid movie
...
//   const prevMeta = prevItem
//     ? getCachedMovie(prevItem.imdb_id) || await getMovieMetadata(token, prevItem.imdb_id)
//     : null;
//   const nextMeta = nextItem
//     ? getCachedMovie(nextItem.imdb_id) || await getMovieMetadata(token, nextItem.imdb_id)
//     : null;

//   // ✅ Save cache if fetched fresh
//   if (prevItem && prevMeta) setCachedMovie(prevItem.imdb_id, prevMeta);
//   if (nextItem && nextMeta) setCachedMovie(nextItem.imdb_id, nextMeta);
...
// Eoisodes
 ...
 // Episodes data
  ...
  const renderMovieDetail = useCallback(
                ...
                (
                  <View style={{ flexDirection: 'row', height: windowHeight * 0.21, marginTop: 10 }}>
                    ...
                    <View style={{
                      ...
                      borderRadius: 2,
                      height: visibleContentHeight, // indicator container की height
                      overflow: 'hidden',
                    }}>
                      <Animated.View
                        style={{
    ...
    [currentIndex, isVideoPaused, progress, duration]
...
//       meta = await getMovieMetadata(token, imdb_idData);
//       // console.log('moive__call from line 321 , imdb_idData__ ',imdb_idData)
//     console.log('getMovieMetadata___data__from___', meta);
...
//     // ✅ Step 3: Fetch matching movies (can also cache later if needed)
//     const matching = await getMatchingMovies(token, imdb_idData);
// console.log('meta?.matching_movies_from__getMovieMEtadata____',meta?.matching_movies)
//     // ✅ Step 4: Update UI states
//     setMovieData([null, meta, null]);
//     setCurrentSeedId(imdb_idData);
//     saveBookMark_Ref.current = meta?.is_bookmarked;
//     setSelectedMovie(meta?.imdb_id);
//     setMatchingQueue(meta?.matching_movies|| []);
//   };
...
//     console.log('console__fetchNextMovieFromQueue__',prevImdb)   // moive imdb_id
//   try {
//     let queue = [...matchingQueue];
//     console.log('fetchNextMovieFromQueue__matchingQuene',...matchingQueue) // movier data list
//     // 🛑 agar queue already filled hai to naya API call mat karo
//     // if (queue.length) {
//     //   const matching = await getMatchingMovies(token, prevImdb);
//     //   queue = matching?.results || [];
...
//     queue.splice(randomIndex, 1);
//     setMatchingQueue(queue);
// console.log('setMatchingQueue__queue  -- line 338',queue) /// movier array
//     // Metadata sirf selected movie ka lo
//     const meta = await getMovieMetadata(token, nextMovie?.imdb_id);
//     console.log('moive__call from line  404 , nextMovie?.imdb_id ',nextMovie?.imdb_id )    
//     saveBookMark_Ref.current = meta?.is_bookmarked;
...
//   // example: after you set new mid movie
//   const preloadNeighborsMeta = async (queue, midMovie) => {
//     console.log('___queue__data',queue)  // movie list
//     console.log('___midMovie__data',midMovie) //  single movie dta
//     const prevItem = queue[0] || null;
//     const nextItem = queue[1] || null;
//     const [prevMeta, nextMeta] = await Promise.all([
//       prevItem ? getMovieMetadata(token, prevItem.imdb_id) : Promise.resolve(null),
