...
// //   // 🔴🔴🔴 SCROLL FIX STATES 🔴🔴🔴
// //   const outerScrollEnabledRef = useRef(true);
// //   const [isDescriptionScrolling, setIsDescriptionScrolling] = useState(false);
// //   const descriptionScrollRef = useRef(null);
  
// //   // Function to control outer scroll
// //   const setOuterScrollEnabledInstant = useCallback((value) => {
// //     outerScrollEnabledRef.current = value;
...
// //   const isAtTopOfDescription = (contentOffsetY) => {
// //     return contentOffsetY <= 0;
// //   };

// //   const isAtBottomOfDescription = (layoutHeight, contentOffsetY, contentHeight) => {
// //     return layoutHeight + contentOffsetY >= contentHeight - 1;
// //   };

// //   // 🔴🔴🔴 SCROLL EVENT HANDLERS FOR DESCRIPTION 🔴🔴🔴
// //   const handleDescriptionScroll = useCallback((event) => {
// //     const { layoutMeasurement, contentOffset, contentSize } = event.nativeEvent;
    
// //     const atTop = isAtTopOfDescription(contentOffset.y);
// //     const atBottom = isAtBottomOfDescription(layoutMeasurement.height, contentOffset.y, contentSize.height);
    
// //     // If at top or bottom, allow outer scroll
// //     if (atTop || atBottom) {
// //       setOuterScrollEnabledInstant(true);
// //       setIsDescriptionScrolling(false);
// //     } else {
// //       // If in middle, disable outer scroll
// //       setOuterScrollEnabledInstant(false);
// //       setIsDescriptionScrolling(true);
// //     }
// //   }, [setOuterScrollEnabledInstant]);
...
// //   const handleDescriptionScrollEndDrag = useCallback((event) => {
// //     const { contentOffset } = event.nativeEvent;
// //     if (isAtTopOfDescription(contentOffset.y)) {
// //       setIsDescriptionScrolling(false);
// //       setOuterScrollEnabledInstant(true);
// //     }
// //   }, [setOuterScrollEnabledInstant]);

// //   const handleDescriptionMomentumScrollEnd = useCallback((event) => {
// //     const { layoutMeasurement, contentOffset, contentSize } = event.nativeEvent;
...
// //   // Touch event handlers for Android
// //   const handleDescriptionTouchStart = useCallback(() => {
// //     setIsDescriptionScrolling(true);
// //     setOuterScrollEnabledInstant(false);
// //   }, [setOuterScrollEnabledInstant]);

// //   const handleDescriptionTouchEnd = useCallback(() => {
// //     // Small delay to prevent immediate outer scroll
// //     setTimeout(() => {
// //       if (!isDescriptionScrolling) {
// //         setOuterScrollEnabledInstant(true);
// //       }
// //     }, 50);
// //   }, [isDescriptionScrolling, setOuterScrollEnabledInstant]);

// //   // 🔴🔴🔴 GESTURE RESPONDER FOR ANDROID 🔴🔴🔴
// //   const handleStartShouldSetResponder = useCallback(() => {
// //     return true;
// //   }, []);

// //   const handleMoveShouldSetResponder = useCallback((event) => {
// //     const { dy, dx } = event.nativeEvent;
// //     // Only take control if vertical scroll is intended
...
// //               {/* 🔴🔴🔴 FIXED DESCRIPTION SCROLLVIEW - MAIN FIX 🔴🔴🔴 */}
...
// //                     onScrollBeginDrag={handleDescriptionScrollBeginDrag}
// //                     onScrollEndDrag={handleDescriptionScrollEndDrag}
// //                     onMomentumScrollEnd={handleDescriptionMomentumScrollEnd}
...
// //       isSeeking, bookmarkMap, seekPosition, currentTime, 
// //       handleDescriptionScroll, handleDescriptionScrollBeginDrag, 
// //       handleDescriptionScrollEndDrag, handleDescriptionMomentumScrollEnd,
// //       handleDescriptionTouchStart, handleDescriptionTouchEnd,
// //       handleStartShouldSetResponder, handleMoveShouldSetResponder,
// //       handleResponderGrant, handleResponderRelease]
// //   );
...
// //         scrollEnabled={outerScrollEnabledRef.current}
...
// //         // Re-render when scroll state changes
// //         key={isDescriptionScrolling ? 'scrolling' : 'not-scrolling'}
// //       />
...
//   // 🔴🔴🔴 SCROLL FIX STATES AND REFS 🔴🔴🔴
//   const [isDescriptionScrolling, setIsDescriptionScrolling] = useState(false);
//   const outerScrollEnabledRef = useRef(true);
//   const descriptionScrollRefs = useRef({});
  
//   // Function to control outer FlatList scroll
//   const setOuterScrollEnabled = useCallback((enabled: boolean) => {
//     outerScrollEnabledRef.current = enabled;
//     // Immediately update FlatList scroll state
//     if (flatListRef.current) {
...
//   // 🔴🔴🔴 FIXED DESCRIPTION SCROLLING FUNCTION 🔴🔴🔴
// ;
...
//   const handleDescriptionScrollEvents = (itemId) => {
//     console.log("Setting up scroll events for item:", itemId);
//     return {
//       // When touch starts on description
//       onTouchStart: () => {
//         console.log("1")
//          setIsDescriptionScrolling(false);
...
//       // Get scroll events for this description
//       const scrollEvents = handleDescriptionScrollEvents(item.imdb_id || index.toString());
...
//                     // Apply all scroll event handlers
//                     {...scrollEvents}
...
//   // setScrollY(y); // save the scroll value in state
// }}

//   // Detect content height
//   onContentSizeChange={(w, h) => {
//     setDescHeight(h);
//     setDescCanScroll(h > viewHeight); // condition checked
//   }}
...
//     setDescCanScroll(descHeight > height); // condition checked
//   }}
...
//     },
//     [currentIndex, isVideoPaused, progress, isMuted, duration, isShowMuteIcon,
//       isSeeking, bookmarkMap, seekPosition, currentTime, handlerShowMuteImg,
...
//         // 🔴 Use ref value for scroll enabled state
//         scrollEnabled={outerScrollEnabledRef.current}
...
//         // 🔴 Additional props for better Android performance
//         removeClippedSubviews={Platform.OS === 'android'}
//         initialNumToRender={2}
        
//         // 🔴 Re-render when scroll state changes
//         key={`flatlist-${isDescriptionScrolling ? 'desc-scrolling' : 'normal'}`}
//       />
...
useEffect(() => {
  ...
  const fetchNextMovieFromQueue = async (prevImdb) => {
    ...
    try {
      ...
      setSelectedMovie(meta.imdb_id);
      console.log('meta?.is_bookmarked__d__sds', meta?.is_bookmarked, meta?.imdb_id)
  ...
  // useEffect(() => {
  // console.log("Current Movie Updated:", movieData[currentIndex]);
  // }, [currentIndex, movieData]);
  ...
  const handleMomentumScrollEnd = async (event) => {
    ...
    if (newIndex !== currentIndex && !isResettingRef.current) {
      if (newIndex === 0 || newIndex === 2) {
        ...
        // console.log('fweff_fasd', movieData)
        const prevId = movieData[1]?.imdb_id || currentSeedId;
  ...
  const showCommenRankingCheck = () => {
    if (!movieData[currentIndex]?.has_rated && !has_rated_ref.current) {
      ...
      //   console.log(imdb_idData,'__show___data')
      // setthinkModal(false)
  ...
  const checkHasRated = () => {
    if (has_rated_ref.current) {
      console.log("User has rated ✅");
    } else {
      console.log("Still not rated ❌");
    }
  };
  ...
  const handleSeek = useCallback((newProgress: number) => {
    console.log('🎯 handleSeek called with progress:', newProgress);
  ...
  const renderMovieDetail = useCallback(
    ({ item, index }) => {
      if (item) {
        // console.log(item ,  item?.has_commented ?? 'rtro__', item?.has_rated ?? 'rtro__',   has_rated_ref.current ?? 'rtro__' ,   'item.n_coxxxmments___sss')
      }
      if (!item) {
        return renderShimmerEffect();
      }
          ...
          >
            ...
            {Platform.OS == "ios" ? <>
              ...
              <VideoPlayer
                ...
                onLoad={handleVideoLoad}
                onSeek={(position) => {
                  console.log('✅ Native seek completed:', position);
                  console.log(isSeeking, 'osSikking')
