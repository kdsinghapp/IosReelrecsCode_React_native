...
import { View, Text, FlatList, TouchableOpacity, ScrollView, StatusBar, Image, Alert, ActivityIndicator, Platform, LayoutAnimation, UIManager, Animated, PanResponder, Dimensions, StyleSheet } from 'react-native';
...
const RankingScreen = () => {
  ...
  useEffect(() => {
    if (ratedMovie.length > 0) {
      ...
      const newOrder = ratedMovie.map(movie => movie.imdb_id);
      setOrder(newOrder);
  ...
  const RankingMovieList = useCallback(({ item, index }: any) => {
    ...
    const isActive = dragState.id === item.imdb_id;
    const top =
      animatedPositions[item.imdb_id]
    // ||
    //  new Animated.Value(index * ITEM_HEIGHT);
    const translateY = dragState.id === item.imdb_id
      ? animatedPositions[item.imdb_id]
      : new Animated.Value(0)
    const scale = activeScale[item.imdb_id] || new Animated.Value(1);
    const zIndex = activeZIndex[item.imdb_id] || new Animated.Value(1);
    return (
      <Animated.View
        style={[
      ...
      >
        <TouchableOpacity
          {...panResponder(item.imdb_id).panHandlers}
          ...
          >
            ...
            <View style={[styles.iconprimary, { alignSelf: 'flex-end' }]}>
              {Platform.OS == 'ios' ?
                ...
                <LayeredShadowText text={(index + 1)} fontSize={40} marginRight={10} />
              }
            </View>
          </TouchableOpacity>
          ...
          <View style={styles.icons}>
            ...
            <View style={styles.centerContainer}>
              <View style={styles.iconprimary}>
                {index > 0 && (
                  <TouchableOpacity
                    onPress={() => swapItems(item.imdb_id, 'up')}
                    disabled={isSwapping || isActive}
                  >
                    <Image
                ...
                {index < ratedMovie.length - 1 && (
                  <TouchableOpacity
                    onPress={() => swapItems(item.imdb_id, 'down')}
                    disabled={isSwapping || isActive}
                  >
                    <Image
  ...
  return (
    <SafeAreaView style={styles.maincontainer}>
      ...
      renderItem={({ item: id, index }) => {
        const movie = ratedMovie.find(m => m.imdb_id === id);
        if (!movie) return null;
        return <RankingMovieList item={movie} index={index} />;
      }}
      ...
      /* ---------------- FOOTER ---------------- */
      ListFooterComponent={() => {
        /* ❌ Jab tak first list complete nahi */
        if (!isOrderListCompleted) {
          return (
        ...
        /* ✅ First list complete hone ke baad */
        return (
          <View>
