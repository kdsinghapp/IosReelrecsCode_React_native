...
import { View, ScrollView, Image, Text, StyleSheet, Dimensions, Animated, TouchableOpacity, ActivityIndicator, Alert } from 'react-native';
...
import { dummyReviews, movieReact } from '../../discover/movieDetail/MovieDetailData';
...
import GroupActivityFeed from '../../../../component/common/GroupActivityFeed/GroupActivityFeed';
...
// const detailWidth = Dimensions.get('window').width * 0.8;
// const detailWidth = Dimensions.get('window').width * 1;
export default function Watchtogether({ token,
  ...
  const [activitiesHeight, setActivitiesheight] = useState(false);
...
const [delayActionPreference , setDelayActionPreference] = useState(false)
  ...
  const handlePreference = async ({
    ...
    // console.log('check_kre__or___dekho__handlePreference')
  ...
  useEffect(() => {
    const fetchGrouchActivities = async () => {
      // console.log(token, groupId, "fetchGrouchActivities______token___groupId___");

      const response = await getGroupActivitiesAction(token, groupId);

      // console.log(response?.results, "fetchGrouchActivities______response___");
  ...
  useEffect(() => {
    // console.log(likes, dislikes, "likes__dislikes___________")
  }, [likes, dislikes]);
  useEffect(() => {
    const fetchActivity = async () => {
      ...
      groupActivityRef.current = result?.results;
       // console.log(result, "results______movoieefewfeffef")
...
setDelayActionPreference(!delayActionPreference)
  ...
  const watchModalFunc = ({ imdb_id }) => {
    ...
    // console.log(imdb_id, "imdb_id____________")
    setSelectedImdbId(imdb_id)
    // Alert.alert(imdb_id)
    setWatchNow(true)
  }
console.log('call__data_here_from_watchTogerther')
  const getMembersScrocedata = async (token, groupId, groupRecommend, activeIndex, movie_ranked) => {
    ...
    // console.log(imdb_id, "group_details__data")
    ...
    if(filteredResults){
      ...
      // console.log(filteredResults, "__________filteredResults______")
    } catch (error) {
      console.error(error, "getMembersScrocedata Error")
      throw error
    }
  }
