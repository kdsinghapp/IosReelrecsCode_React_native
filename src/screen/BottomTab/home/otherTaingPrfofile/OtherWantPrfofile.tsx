...
//       console.log(`✅ Loaded page ${pageToLoad} (${newResults.length} items)`);
//     } catch (error) {
...
const OtherWantPrfofile = () => {
  ...
  const bothBookMovie = async (pageToLoad = 1, append = false) => {
    ...
    try {
      ...
      console.log('data__arha_haui__ere__', response)
    } catch (error) {
...
const handleLoadMore = () => {
  ...
  console.log(movies, 'movies__imageUri_in__OtherWantPrfofile');
  const [bottomModal, setBottomModal] = useState(false)
