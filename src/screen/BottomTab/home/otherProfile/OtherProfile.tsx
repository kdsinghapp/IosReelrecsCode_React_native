...
const OtherProfile = () => {
  ...
  const [otherUserHistory, setOtherUserHistory] = useState([]);
  ...
  const handleFollowUnfollow = async () => {
    ...
    try {
      if (isFollowing) {
        ...
        // console.log(otherUserData , "otherUserData____unfollowUser")
      } else {
  ...
  const fetchOtherUserDetails = async () => {
    ...
    // console.log(item, "superdata_item")
    // console.log(item?.username, "superdata")
    // console.log(token, "superdata_TOken")
    // if (!token || !item?.username) return;
    try {
  ...
  // console.log(otherUserData, "erewerrff")
  ...
  useEffect(() => {
    const fetchRatedMovies = async () => {
      ...
      try {
        ...
        console.error('❌ Error fetching other user rated movies:', err);
      } finally {
  ...
  useEffect(() => {
    const fetchBookmarks = async () => {
      ...
      try {
        ...
        console.error('❌ Error fetching other user bookmarks:', err);
      } finally {
  ...
  useEffect(() => {
    if (hasMore && !loadingFeed && otherUserData?.username) {
      ...
      console.log('fetchFeed("otherprofile__othr_user_name)')
  ...
  const renderHeader = useCallback(() => (
    ...
    <View style={{ marginHorizontal: 14, }}>
      ...
      {/* thrid list */}
      <HorizontalMovieList
        title="Movies/Shows you both want to watch"
  ...
  return combinedData.filter((item, index, self) => {
   ...
   return (
    <SafeAreaView style={styles.container}>
      <CustomStatusBar />
      <HeaderCustom
        title={item?.name || item?.username || 'User Profile'}
        ...
        onBackPressW={() => {
          saveIndex(0)
          navigation.goBack()
          console.log('annna__call_data__OTherProfile_screen')
        }}
      />
      <FlatList
        data={filteredData}
