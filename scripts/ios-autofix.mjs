...
async function buildWithReactNative(simulator) {
  ...
  console.log(`→ Building with react-native for ${simulator.name} (${simulator.udid})…`);
...
async function podInstallIfNeeded(diffText) {
  ...
  if (touched.some((t) => diffText.includes(t))) {
    console.log("→ Detected iOS config changes; running `pod install` …");
...
async function applyPatch(diffText) {
  ...
  console.log(`→ Saved patch to ${patchPath} for debugging`);

  // Validate patch format
  if (!diffText.includes("---") || !diffText.includes("+++")) {
    console.error("Invalid patch format - missing --- or +++ lines");
    console.error("First 500 chars of patch:", diffText.slice(0, 500));
  ...
  if (res.code !== 0) {
    console.error(`git apply failed for patch in ${patchPath}`);
    console.error(res.out || "");
    console.error(res.err || "");
    console.error("\nFirst 20 lines of patch:");
    console.error(cleanedPatch.split("\n").slice(0, 20).join("\n"));
...
async function main() {
  ...
  console.log("🛠️  iOS Auto-Fix loop starting…");
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    if (build.code === 0) {
      console.log("✅ iOS build succeeded!");
      return;
    }

    const combined = redactSecrets(`${build.out}\n\n${build.err}`);
    const logTail = tail(combined, MAX_LOG_CHARS);
    console.log("❌ Build failed. Sending tail of log to Claude for a patch…");
    ...
    if (!patch) {
      console.error("Claude returned empty output. Aborting.");
      break;
    }
    ...
    if (!cleanPatch.startsWith("--- ") && !cleanPatch.includes("\n--- ")) {
      console.error("Claude did not return a unified diff. Output was:\n", patch.slice(0, 500));
      break;
    }
    ...
    if (!allow.ok) {
      console.error(
        `Patch touches a non-allowlisted path (${allow.offending}). Aborting for safety.`
      );
      break;
    }
    ...
    try {
      ...
      console.log("✅ Patch applied & committed. Rebuilding…");
    } catch (err) {
  ...
  console.error("\n🚫 Auto-fix loop stopped.");
  if (SHOULD_OPEN_XCODE_ON_LAST_FAIL) {
    console.log("Opening Xcode to inspect errors…");
